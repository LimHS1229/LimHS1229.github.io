...
{% assign crowd_posts = site.posts | where_exp: "post", "post.path contains 'crowd/_posts/'" | sort: "date" | reverse %}

{% if crowd_posts.size > 0 %}
{% for post in crowd_posts %}
...
{% endfor %}
{% else %}
No posts found in `crowd/_posts`.
{% endif %}
