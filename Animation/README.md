...
{% assign animation_posts = site.posts | where_exp: "post", "post.path contains 'Animation/_posts/'" | sort: "date" | reverse %}

{% if animation_posts.size > 0 %}
{% for post in animation_posts %}
...
{% endfor %}
{% else %}
No posts found in `Animation/_posts`.
{% endif %}
